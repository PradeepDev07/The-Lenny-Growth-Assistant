...
│   • Next.js 14 Standalone Server       │     │   • TaskRouter & Hybrid RAG (BM25)     │
│   • Multi-stage Node 20 Alpine         │     │   • Model Providers (Gemini / Ollama)  │
│   • Port: $PORT (binds to 0.0.0.0)     │     │   • PostgreSQL / SQLite Persistence    │
│   • Dockerfile.vercel / vercel.json    │     │   • Sandboxed Artifact Generation      │
└────────────────────────────────────────┘     └────────────────────────────────────────┘
```
...
| :--- | :--- | :--- | :--- |
| `NEXT_PUBLIC_API_URL` | **Build & Runtime** | `https://lennygrowth.pradeepleadsystems.in` | Base URL of the FastAPI backend. **Must be provided at build time** for client-side JavaScript inlining. |
...
> [!IMPORTANT]
> **Build-Time Inlining:** Next.js bakes `NEXT_PUBLIC_*` variables into client-side JS bundles during `npm run build`. If building via Docker, pass `--build-arg NEXT_PUBLIC_API_URL="https://your-backend-domain.com"`.
...
5. In **Environment Variables**, add:
   ```text
   NEXT_PUBLIC_API_URL = https://lennygrowth.pradeepleadsystems.in
   ```
6. Click **Deploy**.
...
```json
{
  ...
  "rewrites": [
    { "source": "/api/:path*", "destination": "https://lennygrowth.pradeepleadsystems.in/api/:path*" },
    { "source": "/health", "destination": "https://lennygrowth.pradeepleadsystems.in/health" },
    { "source": "/config", "destination": "https://lennygrowth.pradeepleadsystems.in/config" }
  ]
}
```
...
docker build \
  -f Dockerfile.vercel \
  --build-arg NEXT_PUBLIC_API_URL="https://lennygrowth.pradeepleadsystems.in" \
  -t frontend-vercel .
```
